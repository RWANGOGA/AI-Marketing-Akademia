...
import { PRODUCTS as MarketingProducts, ICONS } from "../../_lib/marketingData";
...
export default async function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const mProd = MarketingProducts.find(p => p.id === resolvedParams.id) || MarketingProducts[0];
...
export async function generateStaticParams() {
  try {
    ...
    return res.rows.map(p => ({ id: p.id }));
  } catch (err) {
    return MarketingProducts.map(p => ({ id: p.id }));
  }
}
