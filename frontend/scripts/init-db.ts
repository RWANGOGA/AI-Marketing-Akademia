...
import { NEWS_POSTS } from '../src/app/(marketing)/_lib/marketingData';
...
async function initDB() {
  ...
  try {
    ...
    console.log('Seeding marketing news posts...');
    for (const np of NEWS_POSTS) {
      await pool.query(
        'INSERT INTO content (id, type, title, status, date, excerpt, image_url, tag) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (id) DO NOTHING',
        [np.id, 'news_post', np.title, 'published', np.date, np.excerpt, np.image, np.tag]
      );
    }
